...
We would like
<br>
I usually structure my data-processing workflow such that I can run a single process via command line (for example `python scripty/process-raw-data.py`). `click`, `fire` and `docopt` provide neat functionalities to convert your scripts into interactive command-line interfaces.
